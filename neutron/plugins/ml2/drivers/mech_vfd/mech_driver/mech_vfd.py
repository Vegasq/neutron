...
class VfdMechanismDriver(mech_agent.SimpleAgentMechanismDriverBase):
    ...
    def bind_port(self, context):
        ...
        #AGENT_TYPE_NIC_SWITCH
        LOG.error("OVS agents: %s" % context.host_agents(constants.AGENT_TYPE_OVS))
        LOG.error("NICS agents: %s" % context.host_agents(constants.AGENT_TYPE_NIC_SWITCH))
        for agent in context.host_agents(constants.AGENT_TYPE_NIC_SWITCH):
            LOG.error("Checking agent: %s", agent)
            if agent['alive']:
                for segment in context.segments_to_bind:
                    LOG.error("Segement to bind: %s" % segment)
                    if self.try_to_bind_segment_for_agent(context, segment,
                                                          agent):
                        return
            else:
    ...
    def _get_vif_details(self, segment):
        ...
        if network_type == p_const.TYPE_FLAT:
            ...
            raise Exception("Excp: unknown network_type = %s" % network_type)
