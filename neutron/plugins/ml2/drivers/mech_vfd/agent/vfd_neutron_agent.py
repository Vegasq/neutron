...
VFD_AGENT_BINARY = "neutron-vfd-agent"
# MACVTAP_FS = "/sys/class/net/"
EXTENSION_DRIVER_TYPE = "vfd"

config.register_macvtap_opts()


class VfdRPCCallBack(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def network_delete(self, context, **kwargs):
        LOG.debug("VFD agent network_delete received")
    ...
    def port_update(self, context, **kwargs):
        port = kwargs['port']
        LOG.debug("VFD agent port_update received for port %s ", port)
...
class VfdManager(amb.CommonAgentManagerBase):
    ...
    def validate_interface_mappings(self):
        LOG.error("validate_interface_mappings")
    ...
    def ensure_port_admin_state(self, device, admin_state_up):
        LOG.error("Setting admin_state_up to %s for device %s",
    ...
    def get_agent_configurations(self):
        LOG.error("get_agent_configurations")
        return {'interface_mappings': self.interface_mappings}

    def get_agent_id(self):
        LOG.error("get_agent_id")
    ...
    def get_extension_driver_type(self):
        LOG.error("get_extension_driver_type")
        return EXTENSION_DRIVER_TYPE

    def get_rpc_callbacks(self, context, agent, sg_agent):
        LOG.error("get_rpc_callbacks")
        return VfdRPCCallBack(context, agent, sg_agent)

    def get_agent_api(self, **kwargs):
        LOG.error("get_agent_api")
        pass

    def get_rpc_consumers(self):
        LOG.error("get_rpc_consumers")
...
def main():
    ...
    interface_mappings = [ ["ens3", "admin"] ]

    manager = VfdManager(interface_mappings)
