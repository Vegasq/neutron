...
class DhcpDnsProxy(dhcp.DhcpLocalProcess):
    ...
    def is_dhcrelay_pid(self, path):
        if self.conf.dhcrelay_path in open(path).read():
            return True
        return False
    ...
    def is_dhcp_active(self):
        ...
        pids = [self.get_dhcp_pid(version=DhcpDnsProxy.DHCPv4),
                self.get_dhcp_pid(version=DhcpDnsProxy.DHCPv6)]
        ...
        for pid in pids:
            if(pid and
               os.path.isdir('/proc/%s/' % pid) and
               self.is_dhcrelay_pid('/proc/%s/cmdline' % pid)):
                return True
        return False
